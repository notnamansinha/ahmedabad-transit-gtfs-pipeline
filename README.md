...
A verified dataset covering the public transit systems of Ahmedabad — Bus Rapid Transit (BRT), Municipal Bus Service, and the City Metro. The dataset is built from authoritative municipal sources with cross-source validation.
...
- **Data Ingestion**: Python scripts to aggregate unstructured municipal APIs and HTML endpoints into raw JSON.
- **Topological Matching**: Aligns transit nodes across BRT, Metro, and bus services using OpenStreetMap coordinates.
- **Validation**: Enforces strict JSON Schema validation to guarantee data integrity for downstream tools like GraphHopper and OpenTripPlanner.
